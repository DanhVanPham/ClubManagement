...
public class ManageGroupController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (count == 0) {
                request.setAttribute("LIST_GROUP", new ArrayList<>());
            } else {
                int pageSize = 4;
                int endPage = 0;
                ...
                List<TblGroup> listPaging = blo.getPaging((index - 1) * pageSize, pageSize);
                request.setAttribute("ENDPAGE", endPage);
                request.setAttribute("LIST_GROUP", listPaging);
            }
            url = SUCCESS;
        } catch (Exception e) {
            log("Error at ManageGroupController: " + e.getMessage());
        } finally {
